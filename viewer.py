...
def get_data(exp_dir, img_height, img_width, img_channels, frame_mode, visual_mode):
    ...
    for i, fname in enumerate(sorted_files):
        ...
        if frame_mode == 'dvs':
            ...
            if FLAGS.visual_mode == 'grayscale':
                img = cv2.cvtColor(np.float32(input_img), cv2.COLOR_GRAY2RGB)
...
def plot_steering(img, pred_steer, real_steer):
    ...
    cv2.putText(img, 'GT', (c[0] - r - 60, c[1] - r - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1,
                lineType=cv2.LINE_AA)
    cv2.putText(img, '%0.1f deg' % real_steer, (c[0] - r - 60, c[1] - r - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4,
    ...
    if FLAGS.frame_mode == 'dvs':
        cv2.putText(img, 'DVS', (c[0] + 35, c[1] - r - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1,
                    lineType=cv2.LINE_AA)
    else:
        cv2.putText(img, 'APS', (c[0] + 35, c[1] - r - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1,
                    lineType=cv2.LINE_AA)

    cv2.putText(img, '%0.1f deg' % pred_steer, (c[0] + 35, c[1] - r - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0),
                1, lineType=cv2.LINE_AA)

    return img


def create_video(out_path, width, height):
    # Create Video Writer
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    video_name = os.path.join(out_path, FLAGS.frame_mode + '_output_video.mp4')
    output_video = cv2.VideoWriter(video_name, fourcc, 30, (width, height))

    # Add frame to the output video
    for frame in sorted(os.listdir(out_path)):
        img = os.path.join(out_path, frame)
...
def _main():
    print("Creating image and videos for '{}'.".format(FLAGS.video))
    # Path to images
    exp_dir = os.path.join(FLAGS.video_dir, FLAGS.video)

    # Read ground truth
    steerings_filename = os.path.join(FLAGS.video_dir, FLAGS.video, 'sync_steering.txt')
    try:
    ...
    fname_steer = os.path.join(FLAGS.video_dir, FLAGS.video,
    ...
    for i in range(num_images):
        ...
        if FLAGS.frame_mode == 'dvs':
            ...
            output_path = os.path.join(FLAGS.video_dir, FLAGS.video, "dvs_video")
        ...
        # Show APS only
            ...
            output_path = os.path.join(FLAGS.video_dir, FLAGS.video, "aps_video")
    ...
    create_video(output_path, output_img.shape[1], output_img.shape[0])
