...
class Monitor(mp.Process):
    ...
    def _get(self):
        # read packet header
        data = {'dvs_header': self.sock.recv(28, socket.MSG_WAITALL), 'dvs_timestamp': int(time.time() * 1e6)}
...
class Controller(object):
    ...
    def parse_command(self, command):
        """
        parse string command e.g. string: put /1/1-DAVISFX2/'+str(sensor)+'/aps/ Exposure int 10 (copied from
        https://svn.code.sf.net/p/jaer/code/scripts/python/cAER_utils/imagers_characterization/caer_communication.py)
        """
