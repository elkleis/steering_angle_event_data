...
def compute_percentiles(all_events, inf, sup):
    ...
    idx_median = np.array(np.where(prior_sum >= median_pos))[0][0]
    idx_inf = np.array(np.where(prior_sum >= inf_pos))[0][0]
    idx_sup = np.array(np.where(prior_sum >= sup_pos))[0][0]
...
if __name__ == '__main__':
    ...
    #                     help='Path to frame-based hdf5 files.')
    parser.add_argument('--inf_pos_percentile', default=0.0, help='Inferior percentile for positive events.')
    parser.add_argument('--sup_pos_percentile', default=0.9998, help='Superior percentile for positive events.')
    parser.add_argument('--inf_neg_percentile', default=0.0, help='Inferior percentile for negative events.')
    parser.add_argument('--sup_neg_percentile', default=0.9998, help='Superior percentile for negative events.')
    ...
    for rec in recordings:
        print("Computing percentiles for '{}'".format(rec))
        # Get the data
    ...
    pos_median, pos_inf, pos_sup = compute_percentiles(all_pos_events, float(args.inf_pos_percentile),
                                                       float(args.sup_pos_percentile))
    neg_median, neg_inf, neg_sup = compute_percentiles(all_neg_events, float(args.inf_neg_percentile),
                                                       float(args.sup_neg_percentile))
    print("\nDone all percentiles. \npos_median = {}, pos_inf = {}, pos_sup = {},"
          "neg_median = {}, neg_inf = {}, neg_sup = {}".format(pos_median, pos_inf, pos_sup, neg_median, neg_inf,
                                                               neg_sup))

    np.savetxt(os.path.join(args.source_folder, 'percentiles.txt'),
               [pos_median, pos_inf, pos_sup, neg_median, neg_inf, neg_sup], delimiter=',',
               header='pos_median, pos_inf, pos_sup, neg_median, neg_inf, neg_sup')
