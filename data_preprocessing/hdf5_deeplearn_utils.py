...
class HDF5SeqVisualIterator(object):
    def flow(self, h5f, dataset_key, indexes_key, batch_size, set_length=30, shuffle=True):
        ...
        while b < num_batches:
            ...
            vids = [(np.array(h5f[dataset_key][curr_idx:curr_idx + set_length]) - mean_vid) / std_vid for curr_idx in
                    curr_idxs]
...
def resize_int8(frame, size):
    return misc.imresize(frame, size)
...
def resize_int16(frame, size=(60, 80), method='bilinear', climit=[-15, 15]):
    ...
    return misc.imresize((np.clip(frame, climit[0], climit[1]).astype('float32') + 127), size, interp=method).astype(
        'uint8')
