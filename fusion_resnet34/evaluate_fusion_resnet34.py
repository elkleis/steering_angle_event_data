...
import utils_fusion_resnet34 as utils
from constants import TEST_PHASE
from common_flags_fusion_resnet34 import FLAGS
...
def compute_highest_regression_errors(predictions, real_values):
    """
    Compute the indexes with the highest error
    """
    n_errors = 5
    assert np.all(predictions.shape == real_values.shape)
...
def evaluate_regression(predictions, real_values):
    results = {'evas': compute_explained_variance(predictions, real_values),
               'rmse': compute_rmse(predictions, real_values).tolist(),
               'h_error': compute_highest_regression_errors(predictions, real_values).tolist()}
    return results
...
def _main():
    ...
    # Load weights
    weights_load_path = os.path.join(FLAGS.experiment_rootdir, 'weights', FLAGS.weights_fname)
    try:
    ...
    predictions, ground_truth = utils.compute_predictions_and_gt(model, test_generator, nb_batches, verbose=1)
    ...
    json_dict_fname = os.path.join(Path(os.path.realpath(FLAGS.test_dir)).parent, 'scalers_dict.json')
    ...
    # Evaluate de-transformed predictions: EVA, residuals, and highest errors
    results_dict = {}
    evaluation_dir = os.path.join(FLAGS.experiment_rootdir, 'evaluation_results')
    ...
    for name, pred in baseline_dict.items():
        ...
        utils.write_to_file(results_dict,
                            os.path.join(evaluation_dir, 'test_results.json'))

    # Write predicted and real steerings
    steer_dict = {'pred_steerings': pred_steer.tolist(),
                  'real_steerings': steer_gt.tolist()}
    utils.write_to_file(steer_dict, os.path.join(evaluation_dir, 'predicted_and_real_steerings.json'))
